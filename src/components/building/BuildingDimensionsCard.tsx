...
const craneRowStyle = {
  display: "grid",
  gridTemplateColumns: "minmax(120px, auto) minmax(100px, 1fr) 60px auto auto",
  gap: 10,
  alignItems: "center",
  padding: "10px 0",
      ...
      {open && (
        <div style={bodyStyle}>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            ...
            <SliderField
              label="Ширина"
              value={g.buildingSpanM}
              min={6}
              max={60}
              step={0.1}
              unit="м"
              onChange={(v) => setGeometry({ buildingSpanM: v })}
            />
            <SliderField
              label="Длина"
              value={g.buildingLengthM}
              min={6}
              max={120}
              step={0.1}
              unit="м"
              onChange={(v) => setGeometry({ buildingLengthM: v })}
            />
            <SliderField
              label="Высота"
              value={g.buildingHeightM}
              min={3}
              max={30}
              step={0.1}
              unit="м"
              onChange={(v) => setGeometry({ buildingHeightM: v })}
            />
          </div>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            ...
            <SliderField
              ...
              step={0.1}
              unit="м"
              onChange={(v) => setGeometry({ frameStepM: v })}
            />
            <SliderField
              label="Шаг стоек фахверка"
              value={g.facadePostStepM}
              min={3}
              max={12}
              step={0.1}
              unit="м"
              onChange={(v) => setGeometry({ facadePostStepM: v })}
            />
          </div>
          ...
          <div style={{ display: "grid", gap: 9 }}>
            ...
            <input readOnly value={`${area.toFixed(1)} м²`} style={{ ...sliderInputStyle, width: "100%", background: "#f8fafc", color: "#64748b", textAlign: "left", paddingLeft: 11 }} />
          </div>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              {cranes.hangingCrane.enabled && (
                <div style={{ display: "grid", gap: 10, paddingTop: 4 }}>
                  <div style={fieldRowStyle}>
                    <span style={{ fontSize: 15, color: "#0f172a", fontWeight: 500 }}>Грузоподъёмность, т</span>
                    <input
                      type="number"
                      min={0.5}
                      max={10}
                      ...
                      onChange={(e) => {
                        const v = Number(e.target.value);
                        if (Number.isFinite(v)) setHangingCrane({ capacityT: Math.max(0.5, Math.min(10, v)) });
                      }}
                      style={inputStyle}
                    />
                  </div>
                  <div style={fieldRowStyle}>
                    <span style={{ fontSize: 15, color: "#0f172a", fontWeight: 500 }}>Кол-во кранов</span>
                    <input
                      type="number"
                      min={1}
                      max={10}
                      ...
                      onChange={(e) => {
                        const v = Number(e.target.value);
                        if (Number.isFinite(v)) setHangingCrane({ count: Math.max(1, Math.min(10, v)) });
                      }}
                      style={inputStyle}
                    />
                  </div>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              ...
              {cranes.supportCrane.enabled && (
                <div style={{ display: "grid", gap: 10, paddingTop: 4 }}>
                  ...
                  />
                  <div style={fieldRowStyle}>
                    <span style={{ fontSize: 15, color: "#0f172a", fontWeight: 500 }}>Отметка рельса, м</span>
                    <input
                      type="number"
                      min={1}
                      max={30}
                      ...
                      onChange={(e) => {
                        const v = Number(e.target.value);
                        if (Number.isFinite(v)) setSupportCrane({ railLevelM: Math.max(1, Math.min(30, v)) });
                      }}
                      style={inputStyle}
                    />
                  </div>
