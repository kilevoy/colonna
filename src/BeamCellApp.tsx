...
import {
  ...
  type BeamCellDebugResult,
} from "./calc/beam-cell";

export function BeamCellApp() {
  const [result, setResult] = useState<BeamCellDebugResult>(() =>
    runBeamCellCalculationWithDebug(defaultBeamCellInput),
  );
  ...
  const handleCalc = () => {
    setError(null);
    try {
      setResult(runBeamCellCalculationWithDebug(defaultBeamCellInput));
    } catch (e) {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 22, marginBottom: 4 }}>Балка покрытия</h1>
      ...
      <div style={panelStyle}>
        ...
        <Stat label="Выбранный профиль" value={result.selectedProfile ?? "нет решения"} />
