...
const scenarios = [
  // --- ЛСТК ---
  base("С1: ЛСТК, двускатная, снег 1.5, шаг 1500", {
    materialType: "lstk",
  }),
    ...
    if (isLstk) {
      ...
      if (t && t.length > 0) {
        ...
        for (let i = 0; i < Math.min(3, t.length); i++) {
          const c = t[i];
          console.log(`  ${i+1}. ${c.profile.name} ${c.profile.type} шаг=${c.spacing_mm} K=${f4(c.K)} масса=${f2(c.massPerBuilding_kg)} кг`);
        }
      }
    } else {
...
// Write JSON
fs.writeFileSync('./scripts/purlin_test_results.json', JSON.stringify(results, null, 2), 'utf-8');
console.log("\n✅ JSON written to ./scripts/purlin_test_results.json");
